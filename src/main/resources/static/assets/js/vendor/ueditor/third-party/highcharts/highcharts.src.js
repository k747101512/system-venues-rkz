...
SVGRenderer.prototype = {
	...
	button: function (text, x, y, callback, normalState, hoverState, pressedState, disabledState) {
		...
		return label
			...
			.attr(normalState)
			.css(extend({ cursor: 'static.default' }, normalStyle));
	},
	...
	scroll: function (scrollBy, animation) {
		...
		if (currentPage > 0) {
			...
			this.up.attr({
				...
				.css({
					cursor: currentPage === 1 ? 'static.default' : 'pointer'
				});
			pager.attr({
			...
			this.down.attr({
				...
				.css({
					cursor: currentPage === pageCount ? 'static.default' : 'pointer'
				});
